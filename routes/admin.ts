...
// ---- Users ----
router.post('/users', ...protect, roles('admin', 'teacher'), validate(createUserSchema), asyncHandler(async (req, res) => {
  const { email, password, name, role, class_id } = req.body;

  if (req.user.role === 'teacher' && !['student', 'parent'].includes(role)) {
    return res.status(403).json({ error: 'Учитель может создавать только учеников и родителей', code: ERR.FORBIDDEN });
  }
...
router.get('/users', ...protect, roles('admin', 'teacher'), asyncHandler(async (req, res) => {
