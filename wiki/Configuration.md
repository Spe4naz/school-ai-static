...
Конфигурация в `middleware/logger.ts`:
...
- **Production**: только `warn` и выше
- **Development**: `info` (все запросы)
- **Test/CI**: `silent`
...
Конфигурация TTL в `utils/cache.ts`:
...
const TTL = {
  ...
  USERS: 2 * 60 * 1000,        // 2 мин
};
```

Кэш автоматически очищается при истечении TTL. Инвалидация при записи.

---

## Database Pool

Настройки в `config/database.ts`:

```typescript
pool = new Pool({
  connectionString: url,
  max: 20,                   // Макс. соединений
  min: 2,                    // Мин. соединений (warm pool)
  idleTimeoutMillis: 30000,  // 30 сек простоя
  query_timeout: 10000,      // 10 сек таймаут запроса
});
